...
public:
  ...
  int remove (const T &item_to_remove) {
    ...
    while (marker != 0) {
      if (marker->value() == item_to_remove) {
        if (temp == 0) { // marker is the first element in the list
          if (marker->next() == 0) {
            ...
            //head_ = new Node<T>(marker->value(), marker->next()); Why would it create it, the new head should be the what the marker is pointing at
            head_ = marker->next();
            delete marker;
            marker = 0;
          }
      ...
      //marker = 0;  // reset the marker would not make sense as it has to point somewhere
      temp = marker;
      marker = marker->next();
    }
